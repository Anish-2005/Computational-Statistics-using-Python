...
// Models
const PythonAssignment = require('./models/PythonAssignment');
...
router.route('/python-assignments')
  ...
  .post(async (req, res) => {
    try {
      ...
      const newDoc = new PythonAssignment({ 
        title: title.trim(),
        icon: icon || 'FaCode',
...
router.route('/python-assignments/:id')
  ...
  .put(async (req, res) => {
    try {
      const updated = await PythonAssignment.findByIdAndUpdate(
        req.params.id,
        req.body,
...
//Edit Routes
router.route('/python-assignments/:assignmentId/problems')
  .post(async (req, res) => {
    try {
      ...
      const assignment = await PythonAssignment.findById(req.params.assignmentId);
      if (!assignment) {
...
router.route('/python-assignments/:assignmentId/problems/:problemId')
  .put(async (req, res) => {
    try {
      const { question, code, output } = req.body;
      const assignment = await PythonAssignment.findById(req.params.assignmentId);
...
router.put('/python-assignments/:id/full', async (req, res) => {
  try {
